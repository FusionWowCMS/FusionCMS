...
class Oregoncore implements Emulator
{
    ...
    protected array $queries = [
        ...
        "get_guild_members"         => "SELECT m.guildid guildid, m.guid guid, c.name name, c.race race, c.class class, c.gender gender, c.level level, m.rank member_rank, r.rname rname, r.rights rights FROM guild_member m, guild_rank r, characters c WHERE m.guildid = r.guildid AND m.rank = r.rid AND c.guid = m.guid AND m.guildid = ? ORDER BY r.rights DESC",
        "get_guild"                 => "SELECT guildid guildid, name guildName, leaderguid leaderguid, motd motd, info info, createdate createdate FROM guild WHERE guildid = ?",
