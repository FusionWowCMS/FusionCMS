...
class Skyfire_old implements Emulator
{
    protected $config;
    ...
    protected array $queries = [
        ...
        "get_guild_members" => "SELECT m.guildid guildid, m.guid guid, c.name name, c.race race, c.class class, c.gender gender, c.level level, m.member_rank member_rank, r.rname rname, r.rights rights FROM guild_member m JOIN guild_rank r ON m.guildid = r.guildid AND m.member_rank = r.rid JOIN characters c ON c.guid = m.guid WHERE m.guildid = ? ORDER BY r.rights DESC",
        "get_guild" => "SELECT guildid guildid, name guildName, leaderguid leaderguid, motd motd, info info, createdate createdate, level level, experience experience FROM guild WHERE guildid = ?"
    ];
