...
<div class="grid grid-cols-12 gap-6">
		...
		<div class="flex flex-col gap-6">
				...
				<div class="flex flex-col items-center md:flex-row">
					<div class="ltablet:flex-row ltablet:items-center flex flex-col items-center gap-4 text-center md:items-start text-md-start lg:flex-row lg:items-center">
						<div class="text-center text-md-start">
							<h2 class="font-heading text-xl font-light leading-tight text-muted-800 dark:text-white"><span>{if $response.available}{lang('updates_available', 'admin', [count($response.packages)])}{else}{$response.message}{/if}</span></h2>
							<p class="font-alt text-base font-normal leading-normal leading-normal"><span class="text-muted-400">{lang('last_checked', 'admin', [$last_checked])} | {lang('last_updated', 'admin', [($last_updated) ? $last_updated : lang('never', 'admin')])}</span></p>
						</div>
					</div>
					...
					<div class="ltablet:flex-row ltablet:items-center ms-auto me-auto flex flex-col gap-6 text-center text-md-start lg:flex-row lg:items-center mt-4 mt-md-0 me-md-0">
						<div class="flex-1">
							<a href="{$releases_url}" target="_blank"><button type="button" class="btn btn-large is-button rounded is-button-default"><span>{lang('view_on_github', 'admin')}</span></button></a>
							...
							{if $response.available}
								<button type="button" class="btn btn-large is-button rounded bg-primary-500 dark:bg-primary-500 hover:enabled:bg-primary-400 dark:hover:enabled:bg-primary-400 text-white hover:enabled:shadow-lg hover:enabled:shadow-primary-500/50 dark:hover:enabled:shadow-primary-800/20 focus-visible:outline-primary-400/70 focus-within:outline-primary-400/70 focus-visible:bg-primary-500 active:enabled:bg-primary-500 dark:focus-visible:outline-primary-400 dark:focus-within:outline-primary-400 dark:focus-visible:bg-primary-500 dark:active:enabled:bg-primary-500" __update_install_button__>
									<span>{lang('install_updates', 'admin')}</span>
									<div class="spinner-border spinner-border-sm text-light align-middle" role="status" style="display: none;" __update_install_button_spinner__><span class="sr-only">{lang('loading', 'admin')}</span></div>
								</button>
							{/if}
						</div>
					</div>
			...
			<div class="border-muted-200 dark:border-muted-700 dark:bg-muted-800 relative w-full border bg-white transition-all duration-300 rounded-md p-6" style="display: none;" __update_trace___>
				<div class="mb-8 flex items-center justify-between">
					<h3 class="font-heading text-base font-semibold leading-tight text-muted-800 dark:text-white"><span>{lang('trace_installation', 'admin')}</span></h3>
				</div>
	...
	<div class="ltablet:col-span-8 col-span-12 lg:col-span-8">
		<div class="flex flex-col gap-6">
			{if $response.available}
				...
				<div class="border-muted-200 dark:border-muted-700 dark:bg-muted-800 relative w-full border bg-white transition-all duration-300 rounded-md p-6">
					<div class="mb-8 flex items-center justify-between">
						<h3 class="font-heading text-base font-semibold leading-tight text-muted-800 dark:text-white"><span>{lang('available_updates', 'admin', [count($response.packages)])}</span></h3>
					</div>
							...
							<div class="flex w-full flex-col sm:flex-row sm:items-center">
								<div class="relative mb-4 flex items-center gap-2 px-6 mb-sm-0 sm:px-2 h-10">
									...
									<div>
										...
										<p class="font-alt text-xs font-normal leading-tight text-muted-500 dark:text-muted-400"><span>{lang('author', 'admin')}: <a href="{$item.asset.author.html_url}" target="_blank">{$item.asset.author.name}</a></span></p>
									</div>
								</div>
								...
								<div class="flex flex-col grow justify-end gap-2 sm:flex-row sm:items-center">
									<div class="relative flex h-8 items-center justify-end px-6 sm:h-10 sm:justify-center sm:px-2 w-full sm:w-40">
										<span class="text-muted-400 absolute start-8 top-1/2 mx-auto -translate-y-1/2 text-center font-sans text-xs font-medium uppercase sm:inset-x-0 sm:-top-10 sm:translate-y-0 sm:hidden">{lang('created_at', 'admin')}</span>
										<span class="text-muted-500 dark:text-muted-400 line-clamp-1 font-sans text-sm" data-bs-html="true" data-bs-toggle="tooltip" data-bs-title="{lang('created_at_value', 'admin', [$item.asset.created_at])}">{$item.asset.created_at}</span>
									</div>

									<div class="relative flex h-8 items-center justify-end px-6 sm:h-10 sm:justify-center sm:px-2 w-full sm:w-40">
										<span class="text-muted-400 absolute start-8 top-1/2 mx-auto -translate-y-1/2 text-center font-sans text-xs font-medium uppercase sm:inset-x-0 sm:-top-10 sm:translate-y-0 sm:hidden">{lang('updated_at', 'admin')}</span>
										<span class="text-muted-500 dark:text-muted-400 line-clamp-1 font-sans text-sm" data-bs-html="true" data-bs-toggle="tooltip" data-bs-title="{lang('updated_at_value', 'admin', [$item.asset.updated_at])}">{$item.asset.updated_at}</span>
									</div>

									<div class="relative flex h-8 items-center justify-end px-6 sm:h-10 sm:justify-center sm:px-2">
										<span class="text-muted-400 absolute start-8 top-1/2 mx-auto -translate-y-1/2 text-center font-sans text-xs font-medium uppercase sm:inset-x-0 sm:-top-10 sm:translate-y-0 sm:hidden">{lang('github', 'admin')}</span>
										<a href="{$item.release.html_url}" target="_blank"><button type="button" class="relative font-sans font-normal text-sm inline-flex items-center justify-center leading-5 no-underline h-8 px-3 py-2 space-x-1 border nui-focus transition-all duration-300 disabled:opacity-60 disabled:cursor-not-allowed hover:enabled:shadow-none text-muted-700 bg-white border-muted-300 dark:text-white dark:bg-muted-700 dark:border-muted-600 dark:hover:enabled:bg-muted-600 hover:enabled:bg-muted-50 dark:active:enabled:bg-muted-700/70 active:enabled:bg-muted-100 rounded-md">{lang('view', 'admin')}</button></a>
									</div>
								</div>
							</div>
						{/foreach}
			...
			<div class="border-muted-200 dark:border-muted-700 dark:bg-muted-800 relative w-full border bg-white transition-all duration-300 rounded-md p-6">
				<div class="mb-8 flex items-center justify-between">
					<h3 class="font-heading text-base font-semibold leading-tight text-muted-800 dark:text-white"><span>{lang('update_logs', 'admin')}</span></h3>
				</div>
	...
	<div class="ltablet:col-span-4 col-span-12 lg:col-span-4">
		<div class="ptablet:grid-cols-2 grid gap-6 lg:flex lg:flex-col">
			...
			<div class="border-muted-200 dark:border-muted-700 dark:bg-muted-800 relative w-full border bg-white transition-all duration-300 rounded-md p-6">
				<div class="mb-8 flex items-center justify-between">
					<h3 class="font-heading text-base font-semibold leading-tight text-muted-800 dark:text-white"><span>{lang('frameworks_info', 'admin')}</span></h3>
				</div>

				<div class="space-y-5">
					<div class="flex items-center gap-3">
						<div><h4 class="font-heading text-sm font-light leading-tight text-muted-800 dark:text-white"><span>{lang('fusioncms', 'admin')}:</span></h4></div>
						<div class="ms-auto text-end">v{$cms_version}</div>
					</div>

					<div class="flex items-center gap-3">
						<div><h4 class="font-heading text-sm font-light leading-tight text-muted-800 dark:text-white"><span>{lang('smarty', 'admin')}:</span></h4></div>
						<div class="ms-auto text-end">v{$smarty_version}</div>
					</div>

					<div class="flex items-center gap-3">
						<div><h4 class="font-heading text-sm font-light leading-tight text-muted-800 dark:text-white"><span>{lang('codeigniter', 'admin')}:</span></h4></div>
			...
			<div class="border-muted-200 dark:border-muted-700 dark:bg-muted-800 relative w-full border bg-white transition-all duration-300 rounded-md p-6">
				<div class="mb-8 flex items-center justify-between">
					<h3 class="font-heading text-base font-semibold leading-tight text-muted-800 dark:text-white"><span>{lang('server_info', 'admin')}</span></h3>
				</div>

				<div class="space-y-5">
					<div class="flex flex-column gap-3">
						<div><h4 class="font-heading text-sm font-light leading-tight text-muted-800 dark:text-white"><span>{lang('software', 'admin')}:</span></h4></div>
						<div>{$server_software}</div>
					</div>

					<div class="flex flex-column gap-3">
						<div><h4 class="font-heading text-sm font-light leading-tight text-muted-800 dark:text-white"><span>{lang('modules', 'admin')}:</span></h4></div>
						<div class="overflow-x-hidden overflow-y-auto" style="height: 206px;">
							{if $server_modules && $server_modules|is_array}
								...
								{/foreach}
							{else}
								{lang('unable_list_server_modules', 'admin')}
							{/if}
						</div>
					</div>
				</div>
			...
			<div class="border-muted-200 dark:border-muted-700 dark:bg-muted-800 relative w-full border bg-white transition-all duration-300 rounded-md p-6">
				<div class="mb-8 flex items-center justify-between">
					<h3 class="font-heading text-base font-semibold leading-tight text-muted-800 dark:text-white"><span>{lang('php_info', 'admin')}</span></h3>
				</div>

				<div class="space-y-5">
					<div class="flex flex-column gap-3">
						<div><h4 class="font-heading text-sm font-light leading-tight text-muted-800 dark:text-white"><span>{lang('version', 'admin')}:</span></h4></div>
						<div>{$php_version}</div>
					</div>

					<div class="flex flex-column gap-3">
						<div><h4 class="font-heading text-sm font-light leading-tight text-muted-800 dark:text-white"><span>{lang('extensions', 'admin')}:</span></h4></div>
						<div class="overflow-x-hidden overflow-y-auto" style="height: 206px;">
							{if $php_extensions && $php_extensions|is_array}
								...
								{/foreach}
							{else}
								{lang('unable_list_php_extensions', 'admin')}
							{/if}
						</div>
					</div>
				</div>
