<div class="card">
	<div class="card-header">{lang('edit_teleport_location', 'teleport')}</div>
	...
	<div class="card-body">
		<form role="form" onSubmit="Teleport.save(this, {$teleport_location.id}); return false" id="submit_form">
			...
			<div class="form-group row mb-3">
				...
				<div class="col-sm-10">
					<select class="form-control nui-focus border-muted-300 text-muted-600 placeholder:text-muted-300 focus:border-muted-300 focus:shadow-muted-300/50 dark:border-muted-700 dark:bg-muted-900/75 dark:text-muted-200 dark:placeholder:text-muted-600 dark:focus:border-muted-700 dark:focus:shadow-muted-800/50 peer w-full cursor-pointer appearance-none border bg-white font-sans focus:shadow-lg px-2 pe-9 h-10 py-2 text-sm leading-5 px-3 pe-6 rounded px-3" id="priceType" name="priceType" onChange="Teleport.changePrice(this)">
						<option value="free" {if !$teleport_location.vpCost && !$teleport_location.dpCost && !$teleport_location.goldCost}selected{/if}>{lang('free', 'teleport')}</option>
						<option value="vp" {if $teleport_location.vpCost}selected{/if}>{lang('vp', 'teleport')}</option>
						<option value="dp" {if $teleport_location.dpCost}selected{/if}>{lang('dp', 'teleport')}</option>
						<option value="gold" {if $teleport_location.goldCost}selected{/if}>{lang('gold', 'teleport')}</option>
					</select>
				</div>
			</div>

			<div id="vp_price" {if !$teleport_location.vpCost}style="display:none;"{/if}>
				<div class="form-group row mb-3">
			...
			<div id="dp_price" {if !$teleport_location.dpCost}style="display:none;"{/if}>
				<div class="form-group row mb-3">
			...
			<div id="gold_price" {if !$teleport_location.goldCost}style="display:none;"{/if}>
				<div class="form-group row mb-3">
			...
			<div class="form-group row mb-3">
				...
				<div class="col-sm-10">
					<select class="form-control nui-focus border-muted-300 text-muted-600 placeholder:text-muted-300 focus:border-muted-300 focus:shadow-muted-300/50 dark:border-muted-700 dark:bg-muted-900/75 dark:text-muted-200 dark:placeholder:text-muted-600 dark:focus:border-muted-700 dark:focus:shadow-muted-800/50 peer w-full cursor-pointer appearance-none border bg-white font-sans focus:shadow-lg px-2 pe-9 h-10 py-2 text-sm leading-5 px-3 pe-6 rounded px-3" id="required_faction" name="required_faction">
						<option value="0" {if $teleport_location.required_faction == 0}selected{/if}>{lang('any', 'teleport')}</option>
						<option value="1" {if $teleport_location.required_faction == 1}selected{/if}>{lang('alliance', 'teleport')}</option>
						<option value="2" {if $teleport_location.required_faction == 2}selected{/if}>{lang('horde', 'teleport')}</option>
						<option value="3" {if $teleport_location.required_faction == 3}selected{/if}>{lang('neutral', 'teleport')}</option>
					</select>
				</div>
			</div>

			<button type="submit" class="relative font-sans font-normal text-sm inline-flex items-center justify-center leading-5 no-underline h-8 px-3 py-2 space-x-1 border nui-focus transition-all duration-300 disabled:opacity-60 disabled:cursor-not-allowed hover:enabled:shadow-none text-muted-700 border-muted-300 dark:text-white dark:bg-muted-700 dark:border-muted-600 dark:hover:enabled:bg-muted-600 hover:enabled:bg-muted-50 dark:active:enabled:bg-muted-700/70 active:enabled:bg-muted-100 rounded-md">{lang('save_location', 'teleport')}</button>
	</div>
	</form>
</div>