					...
					{foreach from=$languages item=language key=flag}
						<div class="tab-pane {if $language@iteration == 1}active{/if}" id="article_{$flag}">
							...
							<div class="form-group row mb-3">
								...
								<div class="col-sm-10">
									<textarea name="description_{$flag}" class="tinymce_{$flag} form-control max-h-52 nui-focus border-muted-300 placeholder:text-muted-300 focus:border-muted-300 focus:shadow-muted-300/50 dark:border-muted-700 dark:bg-muted-900/75 dark:text-muted-200 dark:placeholder:text-muted-500 dark:focus:border-muted-700 dark:focus:shadow-muted-800/50 peer w-full border bg-white font-sans transition-all duration-300 focus:shadow-lg disabled:cursor-not-allowed disabled:opacity-75 min-h-[2.5rem] text-sm leading-[1.6] rounded-xl resize-none p-2" id="description_{$flag}" __content__="{$language}" cols="30" rows="10"></textarea>
								</div>
							</div>
						</div>
					{/foreach}
