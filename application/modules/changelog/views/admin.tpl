		...
		<tbody style="border-top:none;">
			<tr>
				{if hasPermission("canAddChange")}
					<td style="font-family:tahoma,geneva,sans-serif;font-size:11px" width="5%"><a href="javascript:void(0)" onClick="Changelog.addChange({$category.id})" data-tip="افزودن آپدیت جدید"><i class="fa-duotone fa-square-plus"></i></a></td>
				{/if}
				<td><b>{$category.typeName}</b></td>
				
				<td>
					{if hasPermission("canEditCategory")}
						<a class="relative font-sans font-normal text-sm inline-flex items-center justify-center leading-5 no-underline h-8 px-3 py-2 space-x-1 border nui-focus transition-all duration-300 disabled:opacity-60 disabled:cursor-not-allowed hover:enabled:shadow-none text-muted-700 border-muted-300 dark:text-white dark:bg-muted-700 dark:border-muted-600 dark:hover:enabled:bg-muted-600 hover:enabled:bg-muted-50 dark:active:enabled:bg-muted-700/70 active:enabled:bg-muted-100 rounded-md pull-right" href="javascript:void(0)" onClick="Changelog.renameCategory({$category.id}, this)" data-tip="{lang("rename_category", "changelog")}"><i class="fa-duotone fa-pen-to-square"></i></a>
					{/if}
...
<div class="box big" id="add_changelog" style="display:none;">
	<div class="card-header"><h2><a href='javascript:void(0)' onClick="Changelog.add()" >{lang("update", "changelog")}</a><i class="fa-duotone fa-regular fa-arrow-right px-2"></i>{lang("new_category", "changelog")}</h2></div>
	<div class="card-body">
