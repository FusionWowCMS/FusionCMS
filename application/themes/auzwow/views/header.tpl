...
<html dir="{if lang('isRTL', 'theme_auzwow') === 1}rtl{else}ltr{/if}" lang="{lang('abbreviation', 'main')}">
	<head>
		<title>{$title}</title>
		...
		<script type="text/javascript">
			...
			var auzwowConfig = {
				...
				countdownDate: '{$theme_configs.config.countdown.date}',
				countdownEnabled: {($theme_configs.config.countdown.enabled) ? 1 : 0}
			};
