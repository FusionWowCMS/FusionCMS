...
use App\Config\Toolbar as ToolbarConfig;
use Kint\Kint;
use MX\CI;
...
class Toolbar
{
    ...
    {
        $this->config = config(ToolbarConfig::class);
     ...
     * @return string
     */
    public function prepare($data, ?RequestInterface $request = null, ?ResponseInterface $response = null)
    {
        /**
    ...
    public function respond()
    {
        ...
        if (CI::$APP->input->get('debugbar') !== null) {
            header('Content-Type: application/javascript');

            ob_start();
            include dirname(__FILE__).'/Toolbar/Views/toolbarloader.js';
            $output = ob_get_clean();
        ...
        if (CI::$APP->input->get('debugbar_time')) {
            get_instance()->load->helper('security');

            $filename = sanitize_filename('debugbar_' . CI::$APP->input->get('debugbar_time'));
            $filename = WRITEPATH . 'debugbar/' . $filename . '.json';

            if (is_file($filename)) {
                // Show the toolbar if it exists
                echo $this->format(file_get_contents($filename));
    ...
    protected function format(string $data, string $format = 'html'): string
    {
        ...
        if ($this->config->maxHistory !== 0 && preg_match('/\d+\.\d{6}/s', CI::$APP->input->get('debugbar_time'), $debugbarTime)) {
            $history = new History();
            $history->setFiles(
                $debugbarTime[0],
        ...
        switch ($format) {
            case 'html':
                ...
                extract($data);
                ob_start();
                include dirname(__FILE__).'/Toolbar/Views/toolbar.tpl.php';
                $output = ob_get_clean();
                break;
