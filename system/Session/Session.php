...
class Session implements SessionInterface
{
     ...
     */
    public function __construct(SessionHandlerInterface $driver, BaseConfig $config)
    {
        $this->driver = $driver;

        $this->config = $config;

        CI::$APP->load->helper('array');
    }
    ...
    protected function configure()
    {
        ...
        $sameSite = CI::$APP->config->item('cookie_samesite');

        $params = [
            'lifetime' => $this->config->expiration,
            'path'     => CI::$APP->config->item('cookie_path'),
            'domain'   => CI::$APP->config->item('cookie_domain'),
            'secure'   => CI::$APP->config->item('cookie_secure'),
